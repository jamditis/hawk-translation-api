...
class Organization(Base):
    ...
    tier: Mapped[str] = mapped_column(String(20), nullable=False, insert_default="instant")
    daily_quota: Mapped[int] = mapped_column(Integer, nullable=False, insert_default=50)
    active: Mapped[bool] = mapped_column(Boolean, insert_default=True)
...
class TranslationJob(Base):
    ...
    status: Mapped[str] = mapped_column(String(30), nullable=False, insert_default="queued")
    ...
    content_type: Mapped[str] = mapped_column(String(30), nullable=False, insert_default="article")
...
class Glossary(Base):
    ...
    terms_json: Mapped[dict] = mapped_column(JSON, nullable=False, insert_default=dict)
    org_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("organizations.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, insert_default=lambda: datetime.now(UTC))
...
class Reviewer(Base):
    ...
    language_pairs_json: Mapped[list] = mapped_column(JSON, nullable=False, insert_default=list)
    active: Mapped[bool] = mapped_column(Boolean, insert_default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, insert_default=lambda: datetime.now(UTC))
...
class WebhookDelivery(Base):
    ...
    attempt_count: Mapped[int] = mapped_column(Integer, insert_default=0)
    last_attempt_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    status: Mapped[str] = mapped_column(String(20), nullable=False, insert_default="pending")
